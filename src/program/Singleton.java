...
//				+ " " + i.getStates() + " // Singleton.setStates()");
	 ...
	 * jelenik meg: <1. ELEM TIPUS (ID)> TO <2. ELEM TIPUS (ID)> ON FIELD(<ID>)
	 * [<1. ELEM TIPUS(ID)> DIR CHANGED: FROM <IRANY X> TO <IRANY Y>] [<1. ELEM
	 * TIPUS(ID)> STATE CHANGED: FROM <ALLAPOT X> TO <ALLAPOT Y>] [<2. ELEM
	 * TIPUS(ID)> DIR CHANGED: FROM <IRANY X> TO <IRANY Y>] [<2. ELEM TIPUS(ID)>
	 * STATE CHANGED: FROM <ALLAPOT X> TO <ALLAPOT Y>],
	 */
